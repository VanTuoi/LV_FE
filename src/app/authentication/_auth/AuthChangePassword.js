...
const AuthChangePassword = ({ title, subtitle, subtext }) => {
    ...
    const { errorChangePassword, errorPassWord, errorRePassWord,
        checkPassword, checkRePassword, changePassword, setJwt } = useChangePassword();

    useEffect(() => {
        const Jwt = searchParams.get('Jwt');                // Lấy jwt từ url
        setJwt(Jwt)
    }, [])
